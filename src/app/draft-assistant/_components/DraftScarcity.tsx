...
export function remainingPositionValue(rows: readonly { value: number | null | undefined; drafted: boolean }[]) {
  ...
  return total > 0 ? Math.round(100 * remaining / total) : null;
}
...
export function DraftScarcity({ position }: { position: Position | "FLEX" }) {
  ...
  return <span className="font-medium tabular-nums" title="Position scarcity: undrafted positive Val divided by all positive Val in this position pool, using the selected projection source. Lower means less value remains. This is not a player survival probability.">Value left: {percent == null ? "—" : `${percent}%`}</span>;
}
