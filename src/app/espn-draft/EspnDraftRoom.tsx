...
export default function EspnDraftRoom() {
  ...
  return <main className="group/draft space-y-5 p-4 md:p-6">
    ...
    <div className="flex flex-wrap items-center gap-3 md:group-has-[[data-state=expanded][data-variant=floating]]/draft:ml-64">
      ...
      {room?.live && <><span className="text-sm">{room.data?.name} · Team {room.live.teamId} · {room.live.picks.filter(pick => pick.playerId !== -1).length} picks</span><Badge variant="outline">ESPN league projections</Badge><Button variant="outline" onClick={stop}>Disconnect</Button></>}
    </div>
