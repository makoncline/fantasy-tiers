...
function Group({ title, children }: { title: string; children: ReactNode }) {
  ...
  return <Collapsible open={open} onOpenChange={setOpen} className="border-t py-2">
    <CollapsibleTrigger className="flex w-full items-center justify-between py-1 text-sm font-semibold">{title}<ChevronDown className={`size-3 transition-transform ${open ? "" : "-rotate-90"}`} /></CollapsibleTrigger>
...
export default function DraftSidebar({ showRecommendations = true }: { showRecommendations?: boolean }) {
  const { draftDetails, draftSlot, picks, userRosterSlots: slots, userPositionCounts, draftContext,
    selectedDraftId, playersAll, positionRows } = useDraftData();
  ...
  return <>
    <Sidebar variant="floating" collapsible="offcanvas" aria-label="Draft and team state" data-testid="draft-sidebar">
      ...
      <div className="space-y-1 pb-2 text-xs" aria-live="polite">
        ...
        <p>Next {complete ? "—" : formatDraftPick(turn.nextOwnPick, teams)}{!complete && turn.betweenOwn != null ? ` · ${turn.betweenOwn} selections between` : ""}</p>
      ...
      <SidebarContent className="px-3 text-xs">
        <Group title="Your team · filled">
          <div className="flex flex-wrap gap-x-3 gap-y-2 tabular-nums">
            {ROSTER_SLOTS.map(slot => {
              const assigned = slots.filter(s => s.slot === slot);
              return assigned.length ? <span key={slot} className="whitespace-nowrap"><span className={slot === "BN" || slot === "FLEX" ? undefined : colors[slot]}>{slot === "BN" ? "Bench" : slot === "DEF" ? "D/ST" : slot}</span> {assigned.filter(s => s.player).length}/{assigned.length}</span> : null;
            })}
          </div>
          {(userPositionCounts.RB ?? 0) + (userPositionCounts.WR ?? 0) > 0 ? <p className="tabular-nums" title="All owned RBs and WRs, including FLEX and bench players">RB/WR ratio <span className="font-medium">{userPositionCounts.RB ?? 0}:{userPositionCounts.WR ?? 0}</span></p> : null}
        </Group>
        <Group title="Navigation">
          <nav aria-label="Draft sections" className="grid grid-cols-2 gap-x-3 gap-y-2 [&_a]:rounded [&_a]:px-1 [&_a]:py-1">
            {showRecommendations && !complete ? <a aria-current={activeSection === "decision-board" ? "location" : undefined} className={activeSection === "decision-board" ? "bg-accent font-medium" : ""} href="#decision-board" onClick={e => { e.preventDefault(); navigate("decision-board"); }}>Recommendations</a> : null}
            {["overall", ...["RB", "WR", "FLEX", "QB", "TE", "DEF", "K"].filter(p => p === "FLEX" ? slots.some(s => s.slot === "FLEX") : positions.some(pos => pos === p)).map(p => p.toLowerCase())].map(pos => <a key={pos} aria-current={activeSection === `players-${pos}` ? "location" : undefined} className={activeSection === `players-${pos}` ? "bg-accent font-medium" : ""} href={`#players-${pos}`} onClick={e => { e.preventDefault(); navigate(`players-${pos}`); }}>{pos === "overall" ? "Overall" : pos === "def" ? "D/ST" : pos.toUpperCase()}</a>)}
            <a aria-current={activeSection === "watch-list" ? "location" : undefined} className={activeSection === "watch-list" ? "bg-accent font-medium" : ""} href="#watch-list" onClick={e => { e.preventDefault(); navigate("watch-list"); }}>Watch list</a>
            <a aria-current={activeSection === "league-needs" ? "location" : undefined} className={activeSection === "league-needs" ? "bg-accent font-medium" : ""} href="#league-needs" onClick={e => { e.preventDefault(); navigate("league-needs"); }}>League needs</a>
        ...
        <section id="league-needs" className="scroll-mt-4 border-t py-2">
          <h3 className="mb-2 text-sm font-semibold">League · open</h3>
          <p>{picks.length}/{teams * rounds || "—"} picks made</p>
          <div className="flex flex-wrap gap-2">{draftContext?.room?.leagueStarterSlotsRemaining ? [...positions, ...(slots.some(s => s.slot === "FLEX") ? ["FLEX" as const] : [])].map(pos => <span key={pos} className="whitespace-nowrap">{pos === "DEF" ? "D/ST" : pos} {draftContext.room.leagueStarterSlotsRemaining[pos] ?? "—"}/{teams > 0 ? teams * slots.filter(slot => slot.slot === pos).length : "—"}</span>) : <span>Room needs unavailable</span>}</div>
        </section>
        <Group title="Your roster">
          {(["Starters", "FLEX", "Bench"] as const).map(group => {
            ...
            if (group === "Bench" && !assigned.some(s => s.player)) return <p key={group} className="text-muted-foreground">Bench 0/{assigned.length}</p>;
            return assigned.length ? <div key={group}><h3 className="mb-1 font-medium text-muted-foreground">{group}</h3>{assigned.map((s, index) => {
              ...
              return <div key={`${s.slot}-${index}`} className="flex items-baseline gap-2 py-1">
                <span className={`w-9 shrink-0 ${p ? colors[p.position] : "text-muted-foreground"}`}>{(p?.position ?? s.slot) === "DEF" ? "D/ST" : p?.position ?? s.slot}</span>
                {p ? <Button variant="link" className="h-auto min-w-0 flex-1 justify-start whitespace-normal p-0 text-left text-xs" onClick={() => setPlayer({ ...detail, ...p, rank: p.rank ?? 0, tier: p.tier ?? 0 })}><PlayerDisplayName playerId={p.player_id} name={p.name} />{row?.sleeper_injury_status ? ` · ${row.sleeper_injury_status}` : ""}</Button> : <span className="flex-1 text-muted-foreground">Empty</span>}
