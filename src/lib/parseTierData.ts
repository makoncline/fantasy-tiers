...
const TIERS_DIR = path.resolve(RANKINGS_DIR, "tiers");
...
export function getTierLastUpdatedDate(
  position: string,
  scoringType: ScoringType
): string | null {
  ...
  try {
    ...
    return metadata.lastUpdated || metadata.lastModified || null;
  } catch (error) {
