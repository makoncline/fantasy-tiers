...
- [2026-06-30] Tiers local generation: Tier files are generated locally from current FantasyPros draft ECR. For seasonal refreshes, run `pnpm run fetch:fp` before `pnpm run fetch:tiers`.
...
- [2026-07-01] Mock draft result artifacts: Store generated mock draft runs under ignored `data/draft-results/<run>/`. Run `pnpm run draft:retrospective -- --result-dir data/draft-results/<run>` to turn pick-level misses and waits into Decision Board improvements.
- [2026-07-01] Draft assistant evaluation bar: Mock bots should create real pressure, and the assistant should produce complete, balanced rosters from every draft slot. Use local quality gates and held-out Sleeper board replays for evaluation.
...
- [2026-07-01] Restored UI draft result: An RB-heavy early build exposed weak WR starters. When improving strategy, keep RB/WR starter balance visible alongside best-overall value instead of optimizing a single blended list.
...
- [2026-09-03] Draft strategy research contract: Before changing draft value, scoring, roster policy, opponent behavior, or recommendation UI, read `docs/draft-strategy-research-contract.md`. Tie the change to an accepted principle and its validation contract. Use `docs/tapthatdraft-research-notes.md` for the detailed source notes.
...
- [2026-07-02] QB floor tuning: Late-QB deadline bonuses can turn bad QBs into false recommendations if they ignore player quality. When tuning 1QB draft scoring, keep QB timing separate from a usable-starter floor and flag negative-value or deep positional-rank QBs.
...
- [2026-07-02] Broken mock rosters are algorithm bugs: The draft algorithm and local quality gates should produce complete rosters with starter and FLEX coverage. If a saved result is broken, tune the algorithm and tests first.
- [2026-07-02] RB anchor tuning: Slot-5 standard mocks can pass roster-count gates but still start WR/WR/TE or wait too long on RB. When a draft opens WR, prefer a close RB anchor in round 2 or 3 before adding another WR/TE unless the value gap is decisive, then validate with a saved UI mock.
- [2026-07-02] No player-specific draft tuning: Algorithm changes must not branch on specific player names or IDs. Convert lessons into generic score components such as position value, roster needs, league demand, tier timing, ADP timing, quality floors, and RB/WR balance.
...
- [2026-07-11] Sleeper strategy replay: `pnpm run draft:replay-sleeper` replays each verified actual pick from only its prior board state. Skip boards that do not contain the original validated league configuration. Count picks excluded by a strategy's hard filter as misses instead of dropping them, or the strict strategy's metrics will be biased upward.
- [2026-07-11] Mock artifact ranking fields: `SimDraftPlayer` must preserve FantasyPros ECR, position rank, and tier from the aggregate bundle. Retrospective tooling reads saved rosters, so dropping these fields silently turns ECR features into sentinel values and makes historical analysis misleading.
